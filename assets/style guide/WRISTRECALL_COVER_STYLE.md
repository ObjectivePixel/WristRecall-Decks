...
1. **Assign one approved preset when the deck is first created.** During creation of a new deck and its initial cover, select exactly one gradient from the approved preset library below. Use its listed hexadecimal values unchanged and in the stated start-to-end order. Save the assigned preset name and colours with the deck metadata so they remain the deck's persistent interface theme. Do not invent an ad hoc gradient when an approved preset applies.
2. **Choose the initial preset uniformly at random.** Give every approved preset an equal chance when assigning a theme to a newly created deck. Do not choose according to the deck topic, background reference, personal preference, convenience, or perceived suitability. Do not default to teal, charcoal, grey, or any other familiar combination. Record the selected preset in the working prompt and deck metadata, and keep it consistent through concept generation, refinement, final delivery, and later cover-image changes. Each newly created deck receives one independent random selection.
3. **Do not reassign the gradient when a cover image changes.** For every replacement, revision, regeneration, or refinement of an existing deck's cover image, read and reuse the gradient already stored in that deck's metadata. Changing the cover image, hero object, background reference, composition, or artwork does not start a new gradient selection. Reroll or replace the gradient only when the user explicitly requests a theme change; otherwise leave its preset name, colour values, and start-to-end order untouched.
...
If this cover is being created with a newly created deck, independently make one uniform random selection from the approved gradient preset library for the surrounding interface theme. Record its preset name and exact start-to-end hexadecimal values in the working prompt and deck metadata, then keep that assignment for the lifetime of the deck. If this is a replacement, revision, regeneration, or refinement of an existing deck's cover image, read and reuse the gradient already stored in the deck metadata; do not select or reroll a gradient merely because the cover image is changing. Only change the stored gradient when the user explicitly requests a theme change. Do not alter its endpoint values or habitually favour teal, charcoal, grey, or any other family. Do not use the gradient to inspire, constrain, or recolour the hero. Choose the hero palette independently for literal accuracy, contrast, readability, and immediate recognition at thumbnail size.
